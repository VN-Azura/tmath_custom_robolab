...
class OrganizationMixin(object):
    ...
    def get_object(self, queryset=None):
        object = super(OrganizationMixin, self).get_object(queryset)
        if object.is_hidden and not self.request.user.is_superuser:
            raise Http404()
        return object
...
class OrganizationList(TitleMixin, ListView):
    ...
    def _get_queryset(self):
        query = super().get_queryset().filter(is_hidden=False)
...
class RequestJoinOrganization(LoginRequiredMixin, SingleObjectMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.object = self.get_object()
        if self.object.is_hidden and not request.user.is_superuser:
            raise Http404()
...
class OrganizationRequestDetail(LoginRequiredMixin, TitleMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if object.organization.is_hidden and not self.request.user.is_superuser:
            raise Http404()
        return object
...
class OrganizationRequestBaseView(LoginRequiredMixin, SingleObjectTemplateResponseMixin, SingleObjectMixin, View):
    ...
    def get_object(self, queryset=None):
        ...
        if not organization.admins.filter(id=self.request.profile.id).exists():
            raise PermissionDenied()
        if organization.is_hidden and not self.request.user.is_superuser:
            raise Http404()
        return organization
...
class OrganizationMemberData(LoginRequiredMixin, OrganizationMixin, SingleObjectMixin, View):
    def get(self, request, *args, **kwargs):
        organization = self.get_object()
        if not self.can_edit_organization(organization):
            return generic_message(request, _("Can't edit organization"),
                                   _('You are not allowed to export data in this organization.'), status=403)

        response = HttpResponse()
        response['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        response['Content-Disposition'] = 'attachment; filename=%s_member_data.xlsx' % organization.name
        contests = Contest.objects.filter(organizations=organization).annotate(total_score=Sum('contest_problems__points'))
        members = organization.members.all().order_by('pk').values_list('user__username', flat=True)
        keys = ['username']
        for contest in contests:
            keys.append(contest.name)
        data = {key: [] for key in keys}
        data['username'] = list(members)
        for contest in contests:
            participations = contest.users.filter(user__user__username__in=members, virtual=0)
            data[contest.name] = [-1] * members.count()
            for part in participations:
                data[contest.name][data['username'].index(part.user.username)] = part.score / contest.total_score if contest.total_score > 0 else 0

        df = pandas.DataFrame(data)
        with pandas.ExcelWriter(response, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name=f'Member of {organization.name}', index=False)
