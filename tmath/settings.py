...
# Install pymysql as MySQLdb early, before any Django imports
try:
...
MIDDLEWARE = (
    ...
    "judge.middleware.RateLimitMiddleware",
...
RATE_LIMIT = {
    "WINDOW_SECONDS": 10,     # cửa sổ tính
    "MAX_HITS": 80,           # ngưỡng request / WINDOW (ví dụ 80 req/10s)
    "BLOCK_SECONDS": 10,      # thời gian block
    "KEY_PREFIX": "rl",
    "TRUSTED_IP_HEADER": "CF-Connecting-IP",  # hoặc "X-Forwarded-For" / None
    "EXEMPT_PATHS": ["/healthz", "/static/", "/media/"],  # bỏ qua những path này
    "WHITELIST_IPS": ["127.0.0.1"],  # IP bỏ qua limit
}
